...
class mpfit:
    ...
    :returns:
        ...
        **.perror**
              ...
              pcerror = mpfit.perror * sqrt(mpfit.fnorm / dof)
                ...
                if USE_MP and self.mp_pool is None:
                    ...
                    if (1000.*eval_time)>self.mp_limit:
                        ...
                        try:
                          res=self.call(fcn, self.params.copy(), functkw, async_call=True)
                          res.get()
                        except:
                          self.mp_pool=None
        ...
        # derivatives or not.
        def call(self, fcn, x, functkw, fjac=None, async_call=False):
                if self.debug:
                        print('Entering call...')
                if self.qanytied:
        ...
        def fdjac2(self, fcn, x, fvec, step=None, ulimited=None, ulimit=None, dside=None,
                ...
                if self.mp_pool is not None:
                  ...
                  for j in range(n):
                          ...
                          step1results.append(self.call(fcn, xp, functkw, async_call=True))
                  step2results=[]
                  for j in range(n):
                          [status, fp]=step1results[j].get()
                          if (fjac is None) and (self.damp>0):
                                  ## Apply the damping if requested.  This replaces the residuals
                                  ## with their hyperbolic tangent.  Thus residuals larger than
                                  ## DAMP are essentially clipped.
                          ...
                          if numpy.abs(dside[j])<=1:
                                  ...
                                  step2results.append((j, fp,
                                                       self.call(fcn, xp, functkw, async_call=True)))
